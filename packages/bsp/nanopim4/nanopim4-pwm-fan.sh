...
fan_initialization () {
	...
	echo $READ_MAX_DUTY_CYCLE > $CHANNEL_FOLDER'duty_cycle'
	...
	while [[ true ]]; do
		...
		if [[ ${TEMPS[-1]} -le ${THERMAL_ABS_THRESH[0]} ]]; then
			...
			if [[ $MODEL -lt ${DC_ABS_THRESH[0]} ]]; then
				...
				echo $MODEL > $CHANNEL_FOLDER'duty_cycle'
			fi
		fi
		sleep $TIME_LOOP
	done
...
set_default () {
	echo 0 > $CHANNEL_FOLDER'duty_cycle'
	echo $PERIOD > $CHANNEL_FOLDER'period'
    ...
    case ${OPT} in
        ...
        C)
                ...
                d)
                    ...
                    if ! [[ $DC_PERCENT_MIN -ge 0 && $DC_PERCENT_MIN -le 70 ]]; then
                ...
                D)
                    ...
                    if ! [[ $DC_PERCENT_MAX -ge 80 && $DC_PERCENT_MAX -le 100 ]]; then
                ...
                t)
                    ...
                    if ! [[ $THERMAL_ABS_THRESH_LOW -ge 0 && $THERMAL_ABS_THRESH_LOW -le 49 ]]; then
                ...
                T)
                    ...
                    if ! [[ $THERMAL_ABS_THRESH_HIGH -ge 50 && $THERMAL_ABS_THRESH_HIGH -le 120 ]]; then
